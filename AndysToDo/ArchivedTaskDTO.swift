...
class ArchivedTaskDTO : TaskCRUDDTO {
    ...
    func loadFakeArchivedTasks() {
        ...
        allTasks = Dynamic([Dynamic(archive1), Dynamic(archive2), Dynamic(archive3), Dynamic(archive4), Dynamic(archive5), Dynamic(archive6), Dynamic(archive7), Dynamic(archive8), Dynamic(archive9), Dynamic(archive11), Dynamic(archive12)])
    }
    ...
    func deArchive(_ task: Task) -> Bool {
        ...
        allTasks!.value.remove(at: Int(allTasks!.value.index(of: Dynamic(task))!))
        return true
    }
}
